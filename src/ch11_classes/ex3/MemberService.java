...
public class MemberService {
    ...
    public void login() {
        ...
        do {
            ...
            // checkResult가 true => 사용가능, checkResult가 false => 사용불가(다시 이메일 입력)
            checkResult = memberRepository.emailCheck(memberEmail);
            if (checkResult) {
                System.out.println("사용 가능한 이메일 입니다.");
            } else {
                System.out.println("이미 사용 중인 이메일 입니다. 다른 이메일을 사용해주세요.");
            }
        } while (!checkResult); // checkResult: 값이 false라면 계속 반복되게
