    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < scores.length; i++) { // 반의 수만큼 반복
            ...
            for (int j = 0; j < scores[i].length; j++) { // 해당 반의 학생 수만큼 반복
                totalSum += scores[i][j]; // 학생점수 합산
            }
        }
        double totalAvg = (double) totalSum / totalStudent; // double 함수는 최대 소수점 15자리까진 오차 x, 그 이후는 반올림
        System.out.println("전체 학생의 평균 점수 " + totalAvg);
    }

    // 타입[][] 변수 = new 타입[][];
    //      ㄴ> 차원 갯수        ㄴ>Ex) [1차원수][2차원수]
    // 다차원 배열의 길이를 다르게 주는 방법 => int[][] scores = new int[2][];
